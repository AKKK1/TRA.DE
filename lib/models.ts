...
const ListingSchema = new mongoose.Schema({
  ...
  images: [{ type: String }],
  wantedItems: [{ type: String, maxlength: 18 }],
  ...
  createdAt: { type: Date, default: Date.now },
  expiresAt: { type: Date },
  serviceWanted: { type: String, default: '', maxlength: 48 },
...
const listingTypeEnum = listingModel?.schema?.path('listingType')?.enumValues || [];

if (listingModel && !listingTypeEnum.includes('EXCLUSIVE')) {
