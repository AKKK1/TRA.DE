...
export async function POST(request: Request) {
  ...
  if (dailyCount >= 15)
    return NextResponse.json({ error: 'Daily limit reached (15 offers)' }, { status: 429 });

  // details details max 3
  const perListingCount = await Offer.countDocuments({
    sender: decoded.id,
    listing: listingId
  });
  if (perListingCount >= 3)
    return NextResponse.json({ error: 'This listing allows up to 3 offers' }, { status: 429 });
